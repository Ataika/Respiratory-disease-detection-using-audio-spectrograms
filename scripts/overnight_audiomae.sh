...
echo "=== [1/6] main run, seed 42 (full 8 epochs, early stopping) ==="
./venv/bin/python scripts/train_icbhi_baseline.py \
  --model-name audiomae --epochs 8 --batch-size 4 --learning-rate 5e-5 \
  --disable-weighted-sampler --seed 42 \
  --checkpoint-path results/checkpoints/final_audiomae_no_sampler_best.pth \
